...
function nameFromFixedArray(view: DataView, offset: number, maxLen: number): string {
  const bytes: number[] = []
...
function parseTableDescriptor(view: DataView, offset: number): LpMetadataTableDescriptor {
  return {
...
async function parseHeader(
  ...
  const expectedHeaderSize =
    minorVersion < LP_METADATA_VERSION_FOR_EXPANDED_HEADER ? HEADER_V1_0_SIZE : HEADER_V1_2_SIZE
...
function parseBlockDevice(view: DataView, offset: number): LpMetadataBlockDevice {
  return {
...
function validateParsedMetadata(
  ...
  for (const extent of extents) {
    if (extent.targetType === LP_TARGET_TYPE_LINEAR && extent.targetSource >= blockDevices.length) {
  ...
  const metadataRegion =
    ...
    BigInt(LP_METADATA_GEOMETRY_SIZE + geometry.metadataMaxSize * geometry.metadataSlotCount) * 2n
  if (metadataRegion > superDevice.firstLogicalSector * BigInt(LP_SECTOR_SIZE)) {
...
export async function readFromImageBlob(blob: Blob): Promise<LpMetadata> {
  ...
  const geomBuf = await common.readBlobAsBuffer(
    blob.slice(primaryGeometryOffset, primaryGeometryOffset + LP_METADATA_GEOMETRY_SIZE)
  )
  ...
  const tablesBuf = await common.readBlobAsBuffer(
    blob.slice(headerPeekOffset + headerSize, headerPeekOffset + headerSize + tablesSize)
  )
  ...
  for (const bd of blockDevices) {
    logDebug(
      `  block device "${bd.partitionName}": size=${bd.size}, flags=0x${bd.flags.toString(16)}`
    )
  }
...
export function getMetadataSuperBlockDevice(metadata: LpMetadata): LpMetadataBlockDevice | null {
...
export async function serializeGeometry(geometry: LpMetadataGeometry): Promise<ArrayBuffer> {
...
function serializeTableDescriptor(view: DataView, offset: number, desc: LpMetadataTableDescriptor) {
...
function serializeNameToFixedArray(view: DataView, offset: number, name: string, maxLen: number) {
...
export async function serializeMetadata(metadata: LpMetadata): Promise<ArrayBuffer> {
  ...
  const tablesParts = [...partitionBufs, ...extentBufs, ...groupBufs, ...blockDevBufs]
 ...
 */
export async function buildWipeSuperImages(metadata: LpMetadata): Promise<WipeSuperImage[]> {
