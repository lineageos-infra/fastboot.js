...
export async function* splitBlob(blob: Blob, splitSize: number) {
  ...
  const headerData = await common.readBlobAsBuffer(blob.slice(0, FILE_HEADER_SIZE))
  ...
  for (let i = 0; i < header.chunks; i++) {
    const chunkHeaderData = await common.readBlobAsBuffer(blob.slice(0, CHUNK_HEADER_SIZE))
